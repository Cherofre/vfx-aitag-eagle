...
## Current Snapshot
- Last Updated: 2026-06-05 17:42
- Phase: v1.0.5 favorite/recent tag suggestion, Codex executable resolution, and hover-help audit implemented locally and synced to installed Eagle plugin
...
- Current Focus: 本地 `master` 工作树已实现但尚未推送/更新 GitHub Release。`plugin.js` 会在 Eagle 4.0 build18+ 可用时读取 `eagle.tag.getStarredTags()` 和 `eagle.tag.getRecentTags()`，缺失或失败时降级为空列表；共享候选 helper 同时服务结果卡片和媒体预览窗。`cli-backends.js` 现在会在 npm `codex.cmd`、显式 npm shim 路径和 WindowsApps 包路径出现时继续深搜并优先选择可直接执行的 `%LOCALAPPDATA%\OpenAI\Codex\bin\...\codex.exe`；无扩展 npm shim、WindowsApps 包路径、以及未解析的普通 `.cmd/.bat` 仍会在分析前阻断。`index.html` 已为设置抽屉、工作台动作、结果过滤器、默认模板、预览和采集窗等 84 个静态控件/入口补齐 `title` 或 `aria-label`，避免非直观参数和高风险操作无说明。已重打本地 `dist\特效AI标签管理-cli.eagleplugin`，包内只有核心 7 个文件，manifest ID `VFX_AI_TAGGER_CLI`，version `1.0.5`，`main.devTools=false`，本地 SHA256 `24F6E71DBF34FF53FFC9C93CF51F25364522207AB762DEF328E32B2BD3414803`。已同步已安装目录 `C:\Users\mumengfei\AppData\Roaming\Eagle\Plugins\VFX_AI_TAGGER_CLI`，备份为 `VFX_AI_TAGGER_CLI.backup-codex-20260605-174128`。
...
- Current Task: Reopen/reload the installed Eagle plugin and smoke-test preview-side tag suggestions, starred/recent ordering, hover titles, and plugin Codex health check inside Eagle.

## Resume Here
- Start with: reopen/reload installed plugin `C:\Users\mumengfei\AppData\Roaming\Eagle\Plugins\VFX_AI_TAGGER_CLI` and smoke-test in Eagle before deciding whether to update the public `v1.0.5` Release again.
...
- Watch out for: this local package has not been pushed, tagged, or uploaded to GitHub Release. Unrelated untracked docs may still exist (`docs/v1.0.5-tag-system-preview-plan.md`, `docs/vfx-tag-taxonomy-review.md`); do not include/delete them unless explicitly requested.
